...
def get_model_output(model, video_processor, tokenizer, video, video_mask, qs, args):
    if model.config.mm_use_im_start_end:
        qs = DEFAULT_VID_START_TOKEN + ''.join([DEFAULT_IMAGE_TOKEN]*8) + DEFAULT_VID_END_TOKEN + '\n' + qs + ' Please answer concisely based on the video content, and avoid providing any irrelevant or unrelated information.'
    else:
...
def run_inference(args):
    ...
    copy_script_to_output(sys.argv[0], args.output_dir)
    ...
    # # 加载并保持字典的完整结构
    # gt_questions = list(json.load(open(args.gt_file_question, "r")).values())
    # gt_questions = get_chunk(gt_questions, args.num_chunks, args.chunk_idx)

    # gt_answers = list(json.load(open(args.gt_file_answers, "r")).values())
    # gt_answers = get_chunk(gt_answers, args.num_chunks, args.chunk_idx)
    ...
    for sample in tqdm(gt_questions):
        ...
        for fmt in tqdm(video_formats):  # Added this line
            ...
            mask_temp_path = os.path.join(video_mask_folder, f"{id}_output{fmt}")
            print(f'temp_path:{temp_path}, mask_temp_path:{mask_temp_path}')
            if os.path.exists(temp_path):
                ...
                output = get_model_output(model, processor['video'], tokenizer, video_path, video_mask_path, question, args)
...
def copy_script_to_output(script_path, output_dir):
    """
    Copy the current script to the output directory.
    """
